...
def get_gl_entries(filters, accounting_dimensions):
	...
	gl_entries = frappe.db.sql(
		"""
		select
			gl.name as gl_entry, gl.posting_date, gl.account, ifnull(c.customer_name,s.supplier_name) as party,
		...
		from `tabGL Entry` gl left join `tabCustomer` c on c.name = gl.against left join
		`tabSupplier` s on s.name = gl.against
